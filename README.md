...
The Fly machine **stops** when no traveler has the desk open for a few minutes, then starts again on the next visit (first load can take several seconds). Computers do not trade in that empty window. A lobby start time only fires if someone still has the lobby open, so the machine is awake.
